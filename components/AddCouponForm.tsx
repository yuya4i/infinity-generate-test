...
interface AddCouponFormProps {
  onAddCoupon: (coupon: Omit<Coupon, 'id' | 'acquiredAt'>) => void;
  onClose: () => void;
}
 ...
 * クーポン追加フォームコンポーネント
 */
export default function AddCouponForm({ onAddCoupon, onClose }: AddCouponFormProps) {
  const [formData, setFormData] = useState({
    source: '',
    title: '',
    description: '',
    category: 'shopping' as CouponCategory,
    expiresAt: '',
    url: '',
    tags: [] as string[],
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const coupon: Omit<Coupon, 'id' | 'acquiredAt'> = {
